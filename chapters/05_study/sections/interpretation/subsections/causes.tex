...
With this procedure we analyze the top four \acp{TP}, \acp{TN}, \acp{FP} and \acp{FN} of a model which are assigned the highest probabilities.
...
\begin{figure}[htpb]
    ...
    \caption[Study Interpretation: Example for LIME]{\Ac{LIME} scores for a false positive.}\label{fig:study:interpretation:LIME}
\end{figure}
...
Contrary to the transformers' inability to aggregate a sequence regarding its vagueness, it is also possible that the vagueness is captured in the transformers' output, but not uncovered by the downstream classifier.
...
Due to the poor results of our models it is likely that the generated $c_i$s are not linearly separable.
...
This leads to the hypothesis that transformer-based models are not capable to aggregate sequences regarding their vagueness in a fashion that they can be linearly separated by a downstream classifier.
